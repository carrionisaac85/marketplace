...
:root {
–bg: #0F0F0F; –surface: #1A1A1A; –surface2: #222222; –surface3: #2C2C2C;
–text: #F0EDE8; –text2: #888580; –accent: #F5C842; –green: #34D399;
–red: #FF6B6B; –border: #2E2E2E; –radius: 16px;
...
.auth-box { background: var(–surface); border: 1px solid var(–border); border-radius: 20px; padding: 36px; width: 100%; max-width: 400px; }
.auth-logo { font-family: var(–font-display); font-weight: 800; font-size: 28px; margin-bottom: 6px; }
...
.auth-btn { width: 100%; padding: 14px; background: var(–accent); color: #0F0F0F; border: none; border-radius: 10px; font-weight: 800; font-size: 15px; cursor: pointer; font-family: var(–font-display); transition: all 0.15s; margin-top: 4px; }
.auth-btn:hover { background: #f0bc2a; }
...
.search-bar { flex: 1; display: flex; align-items: center; background: var(–surface2); border: 1.5px solid var(–border); border-radius: 100px; padding: 0 16px; gap: 8px; }
...
.btn-primary:hover { background: #f0bc2a; }
...
.notif-dot { display: inline-block; width: 7px; height: 7px; background: var(–red); border-radius: 50%; margin-left: 5px; vertical-align: middle; }
...
.compose-send { padding: 10px 18px; background: var(–accent); color: #0F0F0F; border: none; border-radius: 10px; font-weight: 700; font-size: 13px; cursor: pointer; font-family: var(–font-body); white-space: nowrap; }
...
.submit-btn:hover { background: #f0bc2a; }
...
.btn-delete:hover { border-color: var(–red); background: rgba(255,107,107,0.08); }
...
.convo-item { background: var(–surface); border: 1px solid var(–border); border-radius: 12px; padding: 14px 18px; cursor: pointer; transition: border-color 0.15s; display: flex; align-items: center; gap: 12px; }
.convo-item:hover { border-color: var(–accent); }
.convo-item.unread { border-color: var(–accent); }
.convo-info { flex: 1; }
.convo-with { font-size: 14px; font-weight: 600; margin-bottom: 3px; }
...
.unread-badge { width: 8px; height: 8px; background: var(–accent); border-radius: 50%; flex-shrink: 0; }
...
.modal { background: var(–surface); border-radius: 20px; width: 100%; max-width: 500px; max-height: 90vh; display: flex; flex-direction: column; animation: slideUp 0.25s ease; box-shadow: 0 12px 40px rgba(0,0,0,0.6); border: 1px solid var(–border); }
...
.modal-title-text { font-family: var(–font-display); font-size: 16px; font-weight: 700; }
...
.chat-bubble.mine { background: var(–accent); color: #0F0F0F; align-self: flex-end; border-bottom-right-radius: 4px; }
...
.chat-send { padding: 10px 18px; background: var(–accent); color: #0F0F0F; border: none; border-radius: 10px; font-weight: 700; cursor: pointer; font-family: var(–font-body); }
...
const [offerCompose, setOfferCompose] = useState({ message: “”, price: “” });
...
// Conversations: stored in Firestore as /conversations/{id}
const [conversations, setConversations] = useState([]);
const [chatModal, setChatModal] = useState(null); // { convoId, otherName, wantTitle }
...
// Live conversations for current user
useEffect(() => {
if (!user) return;
...
setOfferCompose({ message: “”, price: “” });
setTimeout(() => setSentOffers(p => ({ …p, [wantId]: false })), 3000);
};

const openChat = async (want, offer) => {
// convo ID is deterministic from the two user IDs
const ids = [user.uid, offer.fromId].sort();
const convoId = `${ids[0]}_${ids[1]}_${want.id}`;
const ref = doc(db, “conversations”, convoId);
// create if doesn’t exist
const snap = await import(“https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js”).then(m => m.getDoc(ref));
if (!snap.exists()) {
await import(“https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js”).then(m =>
m.setDoc(ref, {
...
setChatModal({ convoId, otherName: user.uid === offer.fromId ? (want.user) : offer.from, wantTitle: want.title });
};
...
const hasUnread = conversations.some(c => c.lastMessage && c.updatedAt);
...
{authTab === “signup” && (
<input className=“auth-input” placeholder=“Your name” value={authForm.name} onChange={e => setAuthForm(p => ({ …p, name: e.target.value }))} />
)}
...
<input className=“auth-input” type=“password” placeholder=“Password” value={authForm.password} onChange={e => setAuthForm(p => ({ …p, password: e.target.value }))}
