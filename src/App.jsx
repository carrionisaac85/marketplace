...
.chat-bubble-menu-btn{opacity:0;pointer-events:none;background:none;border:none;color:var(--text2);font-size:18px;cursor:pointer;padding:2px 6px;border-radius:8px;transition:opacity .15s;flex-shrink:0;line-height:1;letter-spacing:2px;font-family:var(--fb)}
.chat-msg-row:hover .chat-bubble-menu-btn{opacity:1;pointer-events:auto}
.chat-bubble-menu-btn:hover{background:var(--surface2);color:var(--text)}
...
// Request notification permission on login (web only — native handled by registerNativePush)
useEffect(() => {
if (user && !Capacitor.isNativePlatform() && "Notification" in window && Notification.permission === "default") {
...
try {
  throw new Error("Admin rules deploy requires Google OAuth — use Firebase CLI instead: firebase deploy --only firestore:rules");
